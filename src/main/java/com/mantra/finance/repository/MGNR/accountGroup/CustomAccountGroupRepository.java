...
public class CustomAccountGroupRepository {
    ...
    public Optional<List<CompleteAccountListDTO>> getCompleteAccountList(Integer companyId) {
        ...
        final String sql = "SELECT ag.ID accountGroupId,l.ID ledgerId,sdl.ID subsidiaryLedgerId,ag.CODE accountGroupCode," +
                ...
                " FROM MGNR.ACCOUNTGROUP ag RIGHT OUTER JOIN MGNR.LEDGER l ON ag.ID = l.ACCOUNTGROUPID" +
                " RIGHT OUTER JOIN MGNR.SUBSIDIARYLEDGER sdl ON l.ID = sdl.LEDGERID " +
                " INNER Join MSYS.ACCOUNTGROUPTYPE agt ON ag.ACCOUNTGROUPTYPEID=agt.ID " +
                " INNER Join MSYS.ACCOUNTNATURE an ON sdl.ACCOUNTNATUREID=an.ID WHERE" +
