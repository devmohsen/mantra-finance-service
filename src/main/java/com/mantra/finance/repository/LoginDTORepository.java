...
public interface LoginDTORepository extends JpaRepository<LoginDTO, Long> {
    @Query(" select new LoginDTO(u.id ,u.companyId.id,u.name,u.password," +
            ...
            "u.personId.personnelCode,u.companyId.companyKeyId.mantraKey) " +
            ...
            " and u.personId.isDeleted=false and u.companyId.companyKeyId.mantraKey=:mantraKey " +
            " and pp.isDeleted=false ")
    Optional<Collection<LoginDTO>> getRoleForLogin(@Param("username") String username, @Param("mantraKey") int mantraKey);
}
